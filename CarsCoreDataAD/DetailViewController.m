...
//    NSUndoManager *carUndoManager = self.car.managedObjectContext.undoManager;
//    
//    NSNotificationCenter *dnc = [NSNotificationCenter defaultCenter];
//    [dnc addObserver:self selector:@selector(undoManagerDidUndo:) name:NSUndoManagerDidUndoChangeNotification object:carUndoManager];
//    [dnc addObserver:self selector:@selector(undoManagerDidRedo:) name:NSUndoManagerDidRedoChangeNotification object:carUndoManager];
//}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([[segue identifier] isEqualToString:@"EditSelectedItem"]) {
        ...
        controller.editedObject = self.car;
        switch (indexPath.row) {
            case 0: {
                controller.editedFieldKey = @"model";
                controller.editedFieldName = NSLocalizedString(@"model", @"display name for model");
            } break;
            case 1: {
                controller.editedFieldKey = @"make";
                controller.editedFieldName = NSLocalizedString(@"make", @"display name for make");
            } break;
            case 2: {
                controller.editedFieldKey = @"year";
                controller.editedFieldName = NSLocalizedString(@"year", @"display name for year");
            } break;
        }
    }
}
